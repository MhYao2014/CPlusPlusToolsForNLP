...
void HashMapWord(char *Word, HASHUNIT **VocabHash) {
    ...
    if (hnow == nullptr) {
        ...
        // 如果是的话(表现为hpre指向nullptr)，那就不用再移动了;
        // 如果不是的话(表现为hpre不指向nullptr)，那hnow就是处在链表中间位置。
        if (hpre != nullptr) {
            hpre->next = hnow->next;
...
void CutVocab(ARRAYUNIT *VocabArray, long long MaxVocab, long long MinCount, long long VocabSize, int IfSaveVocab) {
    ...
    for (long long FinalVocabSize=0; FinalVocabSize < MaxVocab; FinalVocabSize++) {
        if (VocabArray[FinalVocabSize].Count < MinCount) {
            ...
            VocabArray[FinalVocabSize].Word = nullptr;
...
ARRAYUNIT * BuildVocab(FILE *CorpusFile, long long MaxVocab, long long MinCount, int IfSaveVocab=0, int IfSaveCoocur=0) {
    // 变量定义区，第一次阅读可以跳过，
    // 后面用到再反过来看
    int IfNotGet=1;
    ...
    HASHUNIT **VocabHash = InitHashTable(TSIZE);
    ...
    if (IfSaveCoocur == 0) {
        HASHUNIT *htmp= nullptr,*hpre= nullptr;
        for (int i=0;i<TSIZE;i++) {
            if (VocabHash[i] != nullptr) {
                htmp = VocabHash[i];
                while (htmp != nullptr) {
                    free(htmp->Word);
                    hpre = htmp;
                    htmp = htmp->next;
                    free(hpre);
                    hpre = nullptr;//防止hpre变成野指针，随机指向某个内存区域
                }
            }
        }
        free(VocabArray);
...
void ArrayToHashWithID(ARRAYUNIT *VocabArray, HASHUNITID **NewVocabHash){
    ...
    unsigned int hval;
    HASHUNITID *hpre= nullptr, *hnow= nullptr;
    while (VocabArray[VocabSize].Word != nullptr){
        VocabSize += 1;
    }
    for (long long i=0; i < VocabSize; i++) {
        hval = HashValue(VocabArray[i].Word, TSIZE, SEED);
        // 这里不调用HashMapWord,因为我们不需要重新申请内存，但是代码就会很像
        for (   hpre = nullptr,hnow = NewVocabHash[hval];
                hnow != nullptr && scmp(hnow->Word, VocabArray[i].Word)!=0;
                hpre = hnow, hnow = hnow->next );
        if (hnow == nullptr) {
            // hnow为空表明该指针没有指向任何内存，所以需要先开辟内存
            // 然后把hnow指向这个内存。
...
long long HashSearch(char *Word, HASHUNITID **NewVocabHash) {
    ...
    HASHUNITID *htmp = nullptr;
    htmp = NewVocabHash[hval];
    while (htmp != nullptr) {
        // 当两个字符串相同时，
        // 一定是以htmp->Word为标准，因为scmp会以第一个字符串的\0作为比较停止的信号，
        // 所以当Word = "air",而htmp->Word = "aircondition"时，如果把Word当作第一个字符串
        // 就会被判做同一个词，这显然是个bug。
        if (scmp(htmp->Word,Word) == 0) {
            id = htmp->id;
            break;
        } else {
            htmp = htmp->next;
...
int BuildCoocur(FILE *CorpusFile, ARRAYUNIT *VocabArray, char *CoocurOutputFile,int HalfWinWidth, int IfSaveCoocur) {
    ...
    long long VocabSize = 0, *lookup= nullptr, lookupIndex, InWordId, OutWordId, InWordRowNum;
    int *CoocurMatrix = nullptr, lineCounter=0;
    while (VocabArray[VocabSize].Word != nullptr){
        VocabSize += 1;
    }
    // 从VocabArray重新建立一个哈希表，这次不用重新开辟内存
    HASHUNITID ** NewHashVocab = InitHashTableID(TSIZE);
    ArrayToHashWithID(VocabArray, NewHashVocab);
    ...
    while (!feof(CorpusFile)) {
        ...
        if (lineCounter%10000 == 0) {
            fprintf(stderr, "Have processed %d lines.\n", lineCounter);
        }
    }
    if (IfSaveCoocur==1) {
        // 开始保存共现矩阵,利用了VocabArray
        for (int i=0; i <= lookup[lookupIndex-1]; i++) {
            ...
            if (InWordId % 10000 == 0) {
                fprintf(stderr, "\rHave saved %d words.",InWordId);
            }
        }
    }
    return 0;
...
int main(int argc, char **argv) {
    ...
    if (IfBuildVocab) {
        ...
        ARRAYUNIT * VocabArray = BuildVocab(CorpusFile, MaxVocab, MinCount, IfSaveVocab, IfSaveCoocur);
        // 统计共现矩阵，并以txt形式保存(word1, word2, CoocurTimes)三元组
        if (IfBuildCoocur) {
            BuildCoocur(CorpusFile,VocabArray,CoocurOutputFile,HalfWinWidth,IfSaveCoocur);
        }
    }
    return 0;
}